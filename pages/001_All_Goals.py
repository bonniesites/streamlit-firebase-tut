...
# Print the type of st.session_state at the start
#st.write(f"Type of st.session_state at start: {type(st.session_state)}")
...
def render_goal(document, is_journal):
    ...
    col_markdone, col_edit, col_delgoal, col_cat, col_goal, col_due, col_timestamp = st.columns([1, 1.25, 1, 2.5, 3, 2, 2], gap='small')
    ...
    with col_markdone:
        ...
        if 'is_done' in document and document['is_done']:
            ...
            help_text = "Mark goal as achieved"
        if st.button(icon, key=f"mark_done_{document['_id']}", help=help_text, use_container_width=True):
            # Toggle mark as done
...
def query_coll(filter_option, search_term):
    ...
    # if 'is_done' not in query:  # Check if 'is_done' key is present in the query
...
def main():
    ...
    # with search:       
        ...
        with st.sidebar.form('search_form'):
            ...
            #st.write(f'filter_choice: {filter_choice}')
            if filter_choice:
                st.session_state.filter_choice = filter_choice
            # st.write(f'st.session_state.search_term: {st.session_state.search_term}')
            # st.write(f'st.session_state.filter_option: {st.session_state.filter_option}')
            filter_option = query_coll(filter_choice, st.session_state.search_term)
            # Update session variable with selected filter option
...
#filter_query = query_coll(st.session_state['filter_choice'], st.session_state.search_term)
filter_query = query_coll('all', '')

# st.write(f'filter_query: {filter_query}')
...
# st.write(f'goal_list: \n {goal_list}')
goal_list = list(goal_list)
# for goal in goal_list:
        # display_message(f'goal_list: {goal_list}')     
...
# import streamlit_bokeh3_events

# # Create a Bokeh Div element with a JavaScript callback to get screen width
# div = Div(text="Screen width will be shown here", width=200, height=100)
# div.js_on_event("mousemove", CustomJS(code="""
#     const el = document.createElement('div');
#     el.innerText = window.innerWidth;
#     document.body.appendChild(el);
#     """))

# # Use streamlit_bokeh_events to display the Div and capture the event
...
#         screen_width = result["GET_TEXT"]
#         st.write(f"Screen width: {screen_width}")
# else:
#     st.write("Please move your mouse over the widget to get the screen width.")
