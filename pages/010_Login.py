...
SITE_TITLE = f'My Multi App | {PAGE_SUBHEADER}'
...
if st.session_state.form =='signup_form' and st.session_state.username =='':  
    ...
    if signup:
        if '' in [new_username, new_user_email, new_user_pas]:
            ...
            if REDDIT_USERS.find_one({'user' : new_username}):
                st.sidebar.error('Username already exists')
            if REDDIT_USERS.find_one({'email' : new_user_email}):
                ...
                if new_user_pas != user_pas_conf:
                    ...
                    REDDIT_USERS.insert_one({'user' : new_username, 'email' : new_user_email, 'pass' : new_user_pas})
    ...
    if REDDIT_USERS.find_one({'user' : username, 'pass' : user_pas}):
