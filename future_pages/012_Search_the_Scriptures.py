...
with st.form('scripture_search'):
    search_terms = st.text_input('Enter search term:', value='', placeholder='Type search term here')
    ...
    if submitted:
        pattern = r'^[a-zA-Z0-9 ,.\-\']+$'
        if re.match(pattern, search_terms): 
            search_file_name, search_count, search_lines = search_str(file_path, search_terms, MY_CHOICE)      
    else:
...
if submitted:
    ...
    with left1:            
        st.success(f'Results for "{search_terms}": {search_count}') 
    with right1:     
        st.download_button(
            key='1',
