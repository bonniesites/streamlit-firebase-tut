...
if not 'search_term' in st.session_state:
    ...
    st.sidebar.error("Current search_term: ", st.session_state.search_term)
...
ALL_GOALS = load_cursor(GOALS_LIST, {}, st.session_state.sort, st.session_state.srt_order)
...
def add_new_document(_coll, data):
    try:
        ...
        MSG_CONTAINER.success(f'Goal added successfully with id: {new_id.inserted_id}')
        return new_id
        st.rerun()  # Refresh the goal journal
    except Exception as e:
        # Handle any exceptions that occur during the insertion process
        MSG_CONTAINER.warning(f'Failed to save goal: {e}')
        return None
...
def check_existing_doc(coll, new_data):    
    ...
    if existing_data:
        MSG_CONTAINER.warning("Data already exists in the collection, not saved.")
    else:
        MSG_CONTAINER.success("Data does not exist in the collection. Data saved!")
